...
def perturbation(train_loader, attack_type, perturbation_rate): #perturbation by random
    ...
    elif attack_type == 'node_embedding':
        train_loader = node_embedding_perturbation(train_loader,perturbation_rate)
...
def random_link_perturbation(train_loader, perturbation_rate):
    ...
    for time, snapshot in enumerate(train_loader):
        ...
        n_perturbations = max(round(snapshot.x.shape[0]*perturbation_rate), round(snapshot.edge_index.shape[1]*perturbation_rate))
...
def random_node_perturbation(train_loader, perturbation_rate):
    ...
    for time, snapshot in enumerate(train_loader):
        ...
        for i in attack_nodes:
            ...
            snapshot.x[i] = noise
        features.append(np.array(snapshot.x.tolist()))
...
def edge_attack_perturbation(train_loader, perturbation_rate):
    ...
    for time, snapshot in enumerate(train_loader):
        ...
        #n_perturbations = round(snapshot.x.shape[0] * perturbation_rate)
        n_perturbations = max(round(snapshot.x.shape[0] * perturbation_rate),
                              round(snapshot.edge_index.shape[1] * perturbation_rate))
        model =DICE()
        model.attack(adj, labels=labels, n_perturbations=n_perturbations)
...
def node_embedding_perturbation(train_loader, perturbation_rate):
    edge_indices = []
    edge_weights = []
    for time, snapshot in enumerate(train_loader):
        adj = to_scipy_sparse_matrix(snapshot.edge_index, snapshot.edge_attr).tocsr()
        ...
        model.attack(adj, attack_type="add", n_candidates=500)
        '''
        model.attack(adj, attack_type="add_by_remove",n_perturbations=n_perturbations, n_candidates=n_perturbations)
        #model.attack(adj, attack_type="add", n_candidates=n_perturbations//2, n_perturbations=)
        #model.attack(adj, attack_type="remove",min_span_tree=True, n_candidates=n_perturbations//2)
