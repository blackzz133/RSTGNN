...
class Robust_RGNN(torch.nn.Module):
    ...
    def get_spatial_loss(self, graphs, coe1, coe2, degrees):
        ...
        for t in range(len(graphs)):
            ...
            #node_num = x.shape[0]
            alph1 = scatter(c1, edge_index[1], dim=0, reduce="sum")
            alph1 = torch.norm(alph1, p=2, dim=1)
            alph2 = scatter(c2, edge_index[1], dim=0, reduce="sum")
            ...
            self.graph_spatial_loss += torch.mean((alph1+alph2)/2-degrees[t]/sim1)
