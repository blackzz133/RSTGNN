...
from TGCN.signal import DynamicGraphTemporalSignal
...
class DBLPELoader(object):
    ...
    def _get_targets_and_features(self):
        targets = torch.tensor(self.Labels).transpose(0, 1).numpy()
        self.targets = [i for i in targets]
        self.features = []
        url = 'features.txt'
        ...
        if os.path.getsize(url) > 0:
            ...
            for time in range(len(self.Graphs)):
                feature = Node2Vec(edge_index=torch.tensor(self._edges[time]), num_nodes=self.Graphs[0].shape[0],
                                   embedding_dim=32, walk_length=4,
                                   context_size=2).forward()
